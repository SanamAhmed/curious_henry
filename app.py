...
from RelationBuilder import RelationBuilder
...
def signup():
    ...
    try:
        ...
        session['user']=email
        return render_template('addfiles.html',projectList=session['projectList'], email=email)
        ...
        if request.method == 'POST':
            ...
            if file and allowed_file(file.filename):
                ...
                db = pymysql.connect(app.config["DATABASEIP"], app.config["DB_USER"], app.config["DB_PASSWORD"],
                                     app.config["DATABASE"])
                cur = db.cursor()
                sql = 'INSERT INTO Project_Files (FileName,ProjectName,ProjectUserID,UploadDate,UploadPath,FileEntities,URL) VALUES (%s,%s, %s ,%s,%s,%s,%s)'
                ...
                if (".docx" in file.filename):
                    ...
                    Html_file.close()
                    document_url = "http://localhost:63342/CuriousHenryBeta1.0/static/web/"+ temp + ".html"
                    ...
                    document_url = "http://localhost:63342/CuriousHenryBeta1.0/static/web/viewer.html?file=" + file.filename
                    entityExtractor_ = EntityExtractor(lang,app.config['UPLOAD_PATH_PDF'] + file.filename,pname.strip("'"),document_url,app.config['GOOGLE_API_KEY'],app.config['NLP_API_KEY'],app.config["DATABASEIP"], app.config["DB_USER"], app.config["DB_PASSWORD"],
                         app.config["DATABASE"],file.filename)
                    ...
                    if(searchable):
                        ...
                        document_url = "http://localhost:63342/CurioisHenryBeta1.0/static/web/viewer.html?file=" + file.filename
                ...
                args = (file.filename, pname.strip("'"), session['user'].strip("'"), formatted_date.strip("'"),
                        app.config['UPLOAD_PATH_PDF'],entityExtractor_.getEntities(),document_url)
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if file and allowed_file(file.filename):
                ...
                projectFile_ = ProjectFile(file.filename,pname,session['user'],processed,formatted_date,app.config['UPLOAD_PATH_PDF'],lang)
                ...
                if(projectFile_.lang=="eng"):
                    try:
                        ...
                        return render_template('addfiles.html', email=session['user'],
                                               ...
                                               message="File processing is failed due to error:"+str(e))


                return render_template('addfiles.html', email=session['user'],projectList=session['projectList'],message="File is successfully uploaded. File will be processed in a while")
...
@app.route('/searchRelation_', methods=['POST', 'GET'])
def searchRelation_():
    Nodes = []
    Edges = []
    pname = ""
    ...
    try:
        ...
        print(query)
        print("fname",fname)
        entityRelationExtractor_ = RelationBuilder(pname, query, app.config["DATABASEIP"], app.config["DB_USER"],
                                                  app.config["DB_PASSWORD"],
                                                  app.config["DATABASE"],fname)

        print("In relation building")

        if(len(fname)> 0):
            print("Here in file relation")
            entityRelationExtractor_.allRelation()
        else:
            entityRelationExtractor_.entityRelation()
        print("returned ")
        ...
        Nodes = entityRelationExtractor_.getNodesList()
        Edges = entityRelationExtractor_.getEdgeList()
        print("here in relation builder")
        print(len(Nodes))
        print(len(Edges))
        if (entityRelationExtractor_ != None):
            del entityRelationExtractor_
